...
public class UsersDAO {
	...
	@Transactional
	public boolean save(Process process, String user) {
		TypedQuery<Process> q = em.createQuery(
				"FROM Process s where s.processId=:processId", Process.class);
		q.setParameter("processId", process.getProcessId());

		if (q.getResultList().size() == 0) {
			User userd = this.byName(user);
			process.getUsers().add(userd);	
			
			em.persist(process);
			userd.getProcesses().add(process);
			return true;
		}
		return false;
	}
	...
	public Process byProcessID(String sessionID){
		TypedQuery<Process> q = em.createQuery("FROM Process s where s.processId=:session", Process.class);
