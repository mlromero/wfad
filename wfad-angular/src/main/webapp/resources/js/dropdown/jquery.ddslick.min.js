(function (a) { 
	function g(a, b) { 
		...
		var g = d.siblings(".dd-pointer"), h = a.find(".dd-option").eq(b), k = h.closest("li"), l = c.settings;
		var m = c.settings.data[b]; 
		...
		if (l.showSelectedHTML) { 
			d.html((m.imageSrc ? '<img class="dd-selected-image' + (l.imagePosition == "right" ? " dd-image-right" : "") 
					+ '" src="' + m.imageSrc + '" />' : "") 
					+ (m.text ? '<label class="dd-selected-text"></label>' : "") 
	...
	function h(b) {
		...
		//$( "#comment" ).toggle( "blind" );
		if (f) { 
			d.slideUp("fast"); 			 
			e.removeClass("dd-pointer-up");			
			//document.getElementById("comment").style.height="60px";			
		} else { 
			d.slideDown("fast"); 
	...
	var b = {}; 
	var c = {data: [], keepJSONItemsOnTop: false, width: 260, height: null, background: "#eee", selectText: "", defaultSelectedIndex: null, truncateDescription: true, imagePosition: "left", showSelectedHTML: true, clickOffToClose: true, onSelected: function () { } }; 
	var d = '<div class="dd-select"><input class="dd-selected-value" type="hidden" /><a class="dd-selected" style="padding-left: 0px; padding-top: 0px; padding-right: 0px; padding-bottom: 0px; "></a><span class="dd-pointer dd-pointer-down"></span></div>';
	var e = '<ul class="dd-options"></ul>';
	var f = '<style id="css-ddslick" type="text/css">' 
		...
		+ ".dd-option{padding:10px; display:inline; border-bottom:solid 1px #ddd; overflow:hidden; text-decoration:none; color:#333; cursor:pointer;-webkit-transition: all 0.25s ease-in-out; -moz-transition: all 0.25s ease-in-out;-o-transition: all 0.25s ease-in-out;-ms-transition: all 0.25s ease-in-out; }" 
		+ ".dd-options > li:last-child > .dd-option{ border-bottom:none; display:inline;}" 
		...
		return this.each(function () { 
			...
			if (!f) { 
				...
				a.each(b.data, function (a, c) { 
					...
					m.append("<li>" + '<a class="dd-option" style="padding-left: 0px; padding-top: 0px; padding-right: 0px; padding-bottom: 0px; ">' 
							+ (c.value ? ' <input class="dd-option-value" type="hidden" value="' + c.value + '" />' : "") 
							+ (c.imageSrc ? ' <img class="dd-option-image style="padding-top: 0px; padding-left: 0px; padding-bottom: 0px;padding-right: 0px;"' 
	...
	b.open = function () { 
		return this.each(function () { var b = a(this), c = b.data("ddslick"); if (c) h(b); }); 
	}; 
		
	b.close = function () { 
		return this.each(function () { var b = a(this), c = b.data("ddslick"); if (c) i(b); }); 
	}; 
